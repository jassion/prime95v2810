...
// Get the interval user-settable parameter for seconds that must have elapsed since the last time the date was output 
...

	{
		int fd1;
		char MYLOG[80]={0};
		strcpy(MYLOG,"mylog.txt");
		
//		HANDLE thread_D = GetCurrentThread ();
		char msg[100]={0};
		char stop_Tstr[30]={0};
		sprintf(msg,"Worker %d stopped!!\n",ld->thread_num);
			
		switch(stop_reason)
		{
			case STOP_ESCAPE:
				strcpy(stop_Tstr,"STOP_ESCAPE");
			case STOP_OUT_OF_MEM:
				strcpy(stop_Tstr,"STOP_OUT_OF_MEM");
			case STOP_FILE_IO_ERROR:
				strcpy(stop_Tstr,"STOP_FILE_IO_ERROR");
			case STOP_FATAL_ERROR:
				strcpy(stop_Tstr,"STOP_FATAL_ERROR");
			case STOP_ABORT:
				strcpy(stop_Tstr,"STOP_ABORT");
			case STOP_WORKER:
				strcpy(stop_Tstr,"STOP_WORKER");
			case STOP_PAUSE:
				strcpy(stop_Tstr,"STOP_PAUSE");
			case STOP_WORK_UNIT_COMPLETE:
				strcpy(stop_Tstr,"STOP_WORK_UNIT_COMPLETE");
			case STOP_PRIORITY_WORK:
				strcpy(stop_Tstr,"STOP_PRIORITY_WORK");
			case STOP_BATTERY:
				strcpy(stop_Tstr,"STOP_BATTERY");
			case STOP_REREAD_INI:
				strcpy(stop_Tstr,"STOP_REREAD_INI");
			case STOP_RESTART:
				strcpy(stop_Tstr,"STOP_RESTART");
			case STOP_MEM_CHANGED:
				strcpy(stop_Tstr,"STOP_MEM_CHANGED");
			case STOP_NOT_ENOUGH_MEM:
				strcpy(stop_Tstr,"STOP_NOT_ENOUGH_MEM");
		}
		
		gwmutex_lock (&MYLOG_MUTEX);
		fd1 = _open (MYLOG, _O_TEXT | _O_RDWR | _O_CREAT | _O_APPEND, CREATE_FILE_ACCESS);
		if (fd1 < 0) {
		...
		time (&this_time);
		{
			...
			buf[25] = ']';
			buf[26] = '\n';
			(void) _write (fd1, buf, 27);
		}

		if (_write (fd1, msg, (unsigned int) strlen (msg)) < 0) goto fail;
		memset(msg,0,sizeof(msg));
		sprintf(msg,"The stop type is %s !\n",stop_Tstr);
		if (_write (fd1, msg, (unsigned int) strlen (msg)) < 0) goto fail;
		_close (fd1);
...
//			if(stop_reason!=0)
			...
			// Get the interval user-settable parameter for seconds that must have elapsed since the last time the date was output 
				...
				if (_write (fd1, msg, (unsigned int) strlen (msg)) < 0) goto fail;//break;
				memset(msg,0,sizeof(msg));
				sprintf(msg,"The stop type is %s !\n",stop_Tstr);
				if (_write (fd1, msg, (unsigned int) strlen (msg)) < 0) goto fail;//break;
				_close (fd1);
